    ...
    HubWormholeUtilities
{
    ...
    function checkAllowedToWithdraw(
        ...
        (
            uint256 vaultDepositedValue,
            uint256 vaultBorrowedValue
        ...
        require(
            vaultDepositedValue >=
                vaultBorrowedValue +
                    normalizedAmount *
                    indices.deposited *
                    getPriceCollateral(assetAddress) *
     ...
     */
    function allowedToRepay(
        address vaultOwner,
        address assetAddress,
        uint256 assetAmount
    ) internal view returns (bool) {
        VaultAmount memory vaultAmount = getVaultAmounts(
            vaultOwner,
            assetAddress
        );
        ...
        bool check = normalizeAmountTokenBridge(
            denormalizedAmount,
            decimals,
            Round.UP
        ) >= normalizeAmountTokenBridge(assetAmount, decimals, Round.DOWN);
    ...
    function checkAllowedToLiquidate(
        ...
        require(
            ...
            "vault not underwater"
        );

        (, uint256 vaultBorrowedTrueValue) = getVaultEffectiveNotionals(
            vaultOwner,
            false
        );
        ...
        for (uint256 i = 0; i < assetRepayAddresses.length; i++) {
            ...
            require(
                allowedToRepay(vaultOwner, asset, assetRepayAmounts[i]),
                "cannot repay more than has been borrowed"
            );
        ...
        require(
            notionalReceived >= notionalRepaid,
            "Liquidator receipt less than amount they repaid"
        );

        // check to ensure that amount of debt repaid <= maxLiquidationPortion * amount of debt / liquidationPortionPrecision
        require(
            notionalRepaid <=
                (getMaxLiquidationPortion() * vaultBorrowedTrueValue) /
                    getMaxLiquidationPortionPrecision(),
        ...
        require(
            notionalReceived <=
                (getMaxLiquidationBonus() * notionalRepaid) /
                    getCollateralizationRatioPrecision(),
    ...
    function checkVaultHasAssets(
        ...
        require(
            amounts.deposited >= amounts.borrowed + normalizedAmount,
            "Vault does not have required assets"
        );
    }
    ...
    function checkLiquidationInputsValid(
        ...
        require(
            assetRepayAddresses.length == assetRepayAmounts.length,
            "Repay array lengths do not match"
        );
        require(
            assetReceiptAddresses.length == assetReceiptAmounts.length,
            "Repay array lengths do not match"
        );
    }
    ...
    function checkDuplicates(address[] memory assetAddresses) internal pure {
        ...
        for (uint256 i = 0; i < assetAddresses.length; i++) {
            for (uint256 j = 0; j < i; j++) {
                require(
                    assetAddresses[i] != assetAddresses[j],
                    "Address array has duplicate addresses"
                );
            }
        }
    }
