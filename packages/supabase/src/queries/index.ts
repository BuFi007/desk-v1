...
export async function getMetricsQuery(
  ...
  const prevTotal = prevData?.reduce((value: any, item: { value: any; }) => item.value + value, 0);
  const currentTotal = currentData?.reduce(
    (value: any, item: { value: any; }) => item.value + value,
    0,
  );
  ...
  return {
    ...
    result: currentData?.map((record: { date: any; value: number; }, index: any) => {
...
export async function getExpensesQuery(
  ...
  const averageExpense =
    data && data.length > 0
      ? data.reduce((sum: any, item: { value: any; }) => sum + (item.value || 0), 0) / data.length
      : 0;
  ...
  return {
    ...
    result: data.map((item: { value: any; recurring_value: any; }) => ({
      ...item,
      value: item.value,
...
export async function getVaultRecursiveQuery(
  ...
  const subfolders = folderContents?.filter((item: { id: null; }) => item.id === null) ?? [];
  const folderItems = folderContents?.filter((item: { id: null; }) => item.id !== null) ?? [];

  folderItems.forEach((item: any) => items.push({ ...item, basePath }));
  ...
  subFolderContents.map((subfolderContent) => {
    subfolderContent.map((item: any) => items.push(item));
  });
...
export async function getCategoriesQuery(
  ...
  return supabase
    .from("transaction_categories" as any)
...
export async function getTeamSettingsQuery(supabase: Client, teamId: string) {
  return supabase.from("teams" as any).select("*").eq("id", teamId).single();
}
